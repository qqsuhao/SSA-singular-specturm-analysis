...
<span id="eq2.9"></span>
$$
x_{n}=c_{0}+\sum_{k=1}^{\lfloor N / 2\rfloor}\left(c_{k} \cos (2 \pi n k / N)+s_{k} \sin (2 \pi n k / N)\right)
$$
...
<span id="eq2.11"></span>
$$
\left\|\mathbb{X}_{N}\right\|_{\mathrm{F}}^{2} / N=\sum_{k=0}^{\lfloor N / 2\rfloor} \Pi_{x}^{N}(k / N)
$$
...
<span id="eq2.12"></span>
$$
\frac{1}{N} \sum_{j=1}^{N} x_{j+k} x_{j+m} \underset{N \rightarrow \infty}{\longrightarrow} R(k-m),
$$
...
<span id="eq2.13"></span>
$$
x_{n} \sim \sum_{i} a_{k} \cos \left(2 \pi \omega_{k} n\right)+\sum_{i} b_{k} \sin \left(2 \pi \omega_{k} n\right), \quad \omega_{k} \in(0,1 / 2]
$$、
其中，$a_{k} = a\left(\omega_{k}\right), b_{k} = b\left(\omega_{k}\right), b(1 / 2) = 0$，并且$\sum_{k}\left(a_{k}^{2}+b_{k}^{2}\right)$收敛。（注意其中$a(1/2)\neq 0$如果$\omega_k$恰好是1/2）公式[(2.13)](#eq2.13)意味着频谱测量$m_x$集中在频率点